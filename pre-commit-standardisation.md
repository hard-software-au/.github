...
- Confirm parity with existing bootstrap flow and hook outcomes
...
- [x] Document `gitleaks` as a self-service prerequisite (it is `language: system` — not in the venv):
  ...
  - Gitleaks setup documented via local bootstrap flow and manual fallback commands
  - Developers must install individually (either via bootstrap script checks or manual commands) before running hooks locally; not org-wide enforced
...
- [x] Confirm reusable CI workflow is green on `.github` PRs using changed-files mode
- [x] Enforce branch protection in `.github`: pre-commit check must pass before merge
- [x] Gate: do not run `.github/rollout-devops-assets.sh` against any downstream repo until all items above are complete and merged to default branch
- [x] Add local-only files created in this stage to `.gitignore` (e.g., `.venv/`, CI-generated local artifacts)
...
- [ ] Add `bootstrap.sh` to repo root:
  - Template available in `.github/templates/` directory — copy and customize the `PROFILES` list
  - Commit as-is; developers run it once to set up local hooks: `./bootstrap.sh`, then document the command in `README.md`
